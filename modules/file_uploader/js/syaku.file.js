  ...
  jQuery.syakuFileUpload = {
    ...
    swfupload : function(options) {
      ...
      var settings = {
        ...
        delete_data : swfu_delete_data , 
        file_folder : swfu_files_path,
        post_params: {
        ...
        file_size_limit : 1024 * 1024, // KB 단위로 입력
        file_types : "*.*",
        file_types_description : "",
        file_upload_limit : "0",
        file_queue_limit : "0",

        upload_after_handler : null,
        ...
        */
        ...
        button_image_url : swfu_path + "/js/syaku.file.png",
        button_text : "첨부파일",
        ...
        // Flash Settings
        flash_url : swfu_path + "/js/SWFUpload/swfupload.swf",
    ...
    deleteSwfupload : function(swfu,editor) {
      ...
      if (file_upload_multi) {
        ...
        jQuery.ajax({
          url: '/',
          type: 'post',
          data: swfu.settings.delete_data + '&file_orl=' + file_orl,
...
//                swfu.cancelUpload(file_info.file_id, true);
                ...
                if ( jQuery.isFunction( swfu.settings.upload_after_handler ) ) {
                  swfu.settings.upload_after_handler(swfu,$(this).val(),true);
                }
        ...
        jQuery.ajax({
          url: '/',
          type: 'post',
          data: swfu.settings.delete_data + '&file_orl=' + file_info.file_orl,
          error : function(xhr, status, error) {
            if (xhr.readyState != 4 && xhr.status != 200) { alert("오류가 발생하였습니다." + "\n[ajax error]" + xhr.readyState + "JA" + xhr.status); }
          },
          success : function(xml) {
            if (jQuery(xml).find('error').text() != "true") {
              ...
              if ( jQuery.isFunction( swfu.settings.upload_after_handler ) ) {
                swfu.settings.upload_after_handler(swfu,ele_file_orl_val,true);
              }
    ...
    tag_view : function(swfu,file_info,idx) {
      ...
      var tag = "";
      var cls_name = "syaku syaku_" + idx;

      if (idx == undefined) {
        cls_name = "syaku syaku" + file_orl;
      }

      if(/\.swf$/i.test(filename)) {
        tag = "<div><embed class='" + cls_name + "' src=\""+ file+"\" type=\"application/x-shockwave-flash\"></embed>" + filename + "</div>";
      } else if(/\.(wmv|avi|mpg|mpeg|asx|asf|mp3)$/i.test(filename)) {
        tag = "<div><embed class='" + cls_name + "' src=\""+ file+"\" autostart=\"true\" Showcontrols=\"0\"></embed>" + filename + "</div>";
      } else if(/\.(jpg|jpeg|png|gif)$/i.test(filename)) {
        tag = "<img class='" + cls_name + "' src=\""+ file+"\" border=\"0\" />";
      } else {
        tag = "<a class='" + cls_name + "' href='" + file + "'>" + filename + "</a>";
      }
